...
def find_skills():
    return render_template('find-skills.html', user=session.get('user_name'))

@app.route('/teach-skill', methods=['GET', 'POST'])
def teach_skill():
    if 'user_email' not in session:
        flash('Please login first to post a skill!')
    ...
    if request.method == 'POST':
        ...
        user_email = session['user_email'] # NOW USES REAL USER
    ...
    return render_template('teach-skill.html', user=session.get('user_name'))
...
def browse():
    ...
    skills = c.fetchall()
    conn.close()
    return render_template('browse.html', skills=skills, user=session.get('user_name'))
...
def login():
    if request.method == 'POST':
        ...
        if user:
            ...
            session['user_name'] = user[1] # Save name in session
